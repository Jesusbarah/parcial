import math

"""
***************************************************************
@@ ejercicio 1 @@
un metodo python que haga la multiplicacion de 3 numeros
2*4*3 = 24
"""


# start-->
def multiplicar(numero1,numero2,numero3):
    producto = numero1*numero2*numero3
    return producto


"""
***************************************************************
@@ ejercicio 2 @@
la suma de los numeros divisibles entre 3 y 5 del 1000 al 2000
"""


# start-->
def sumaDivTresYCincoPlus():
    i=1000
    suma = 0
    while i<=2000:
        entre3 = i%3
        entre5 = i%5
        if entre3==0 and entre5==0:
            suma +=i
            i+=1
        else:
            i+=1
    result = suma
    return result


"""
***************************************************************
@@ ejercicio 3 @@
encontrar el area y el volumen de un ortoedro
longitud = 10 m
latitud = 6 m
altura = 5 m

area: A = 2(longitud · latitud + longitud · altura + latitud · altura)
volumen: V = longitud · latitud · altura
"""


# start-->
def definicionOrtoedro(longitud,latitud,altura):
    result1 = obtenerArea(longitud,latitud,altura)
    result2 = obtenerVolumen(longitud,latitud,altura)
    result = {"area":result1,"volumen":result2}
    return result


def obtenerArea(n1,n2,n3):
    area = 2*(n1*n2+n1*n3+n2*n3)
    result = area
    return result


def obtenerVolumen(n1,n2,n3):
    volumen = n1*n2*n3
    result = volumen
    return result


"""
***************************************************************
@@ ejercicio 4 @@
el ejercicio numero 3 convertirlo en una clase

"""


# start-->
class Ortoedro:
    def definicionOrtoedro(self,n1,n2,n3):
        area = 2*(n1*n2+n1*n3+n2*n3)
        volumen = n1*n2*n3
        result = {"area":area,"volumen":volumen}
        return result


"""
***************************************************************
@@ ejercicio 5 @@
VentaComputadoras
Computadora
    procesador
    ram
    tarjeta_grafica
    ssd
    costo
    conDescuento
    descuento
    conPlazo
    cuota
"""


class VentaComputadoras:
    def __init__(self):
        self.thislist = []

    def orden(self,computadora):
        tuple = (computadora)
        thislist.append(tuple)
        return tuple

    def totalProcesadorIntel(self):
        i=0
        total=0
        while i<=(len(self.thislist)-1):
            tuple = self.thislist[i]
            tuple2 = tuple(i)
            if "intel"== tuple2[0]:
                total += tuple2[4]
        return total

    def totalRam16ConDescuento(self):
        i=0
        total=0
        while i<=(len(self.thislist)-1):
            tuple = self.thislist[i]
            tuple2 = tuple(i)
            if "intel"== tuple2[0]:
                total += tuple2[4]
        return total


class Computadora:
    pass


"""
***************************************************************
@@ ejercicio 6 @@
colocar este proyecto en github
colocar aca debajo la url
ademas colocar la url en un archivo
github_<nombre>_<codigo>.txt y subirlo a moodle
"""


# github url-->
def getGithubUrl():
    return "https://github.com/Jesusbarah/parcial.git"
